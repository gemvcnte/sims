...
export default function UpdateSelectTeacherCombobox({
  onSelectTeacher,
  selectedTeacher,
}) {
  const [open, setOpen] = React.useState(false);
  const [value, setValue] = React.useState(selectedTeacher || ""); // Set initial value

  const { teachers, error } = useGetAllTeachers();

  useEffect(() => {
    // Update the value when selectedTeacher prop changes
    setValue(selectedTeacher || "");
  }, [selectedTeacher]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value
            ? teachers.find((teacher) => teacher.username === value)?.fullName
            : "Select teacher..."}
